...
export function part1dfs(input?: string[]) {
  const lines = input ?? readInputFromFile(__dirname);
  const grid = lines.map((line) => line.split(""));
  const scores: number[][] = create2dArrayOf(
    grid.length,
    grid[0].length,
  ...
  function isValidMove(from: Point, to: Point) {
    const f = getHeight(grid[from[1]][from[0]]);
    const t = getHeight(grid[to[1]][to[0]]);
    return f >= t - 1;
  }

  // find start node and target node
  function findEndpoints(grid: string[][]): {
    start: Point;
    target: Point;
  } {
