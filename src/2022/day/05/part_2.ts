...
export function part2(input?: string[]) {
  ...
  instructions.forEach((instruction:string) => {
    ...
    // collect all target crates to move as one array
    const move = stacks[from - 1].slice(-num);
    // remove the target crates from the source stack
    stacks[from - 1] = stacks[from - 1].slice(0, -num);
    // add the target crates to the dest stack
    stacks[to - 1] = [...stacks[to - 1], ...move];
  })
